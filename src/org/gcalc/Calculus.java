...
public static String findCriticalPoints(String equation) {
    try {
        ...
        IExpr criticalPoints = evaluator.eval("Select[NSolve[" + firstDeriv.toString() + "==0,x], Element[x/.#,Reals]&]");
        ...
        if (pointsStr.equals("{}")) {
            ...
            for (String solution : solutions) {
                if (!solution.contains("x->")) continue;
                
                try {
                    String xExpr = solution.split("x->")[1].trim();
                    double xValue = evaluator.eval("N(" + xExpr + ")").evalDouble();
                    double secondDerivValue = evaluator.eval("N(" + secondDeriv + "/.x->" + xExpr + ")").evalDouble();
    ...
    public static String integrate(String equation) {
        try {
            ...
            return integral.toString().toLowerCase();
        } catch (Exception e) {
        ...
        try {
            ...
            if (inflectionSolutions.isEmpty()) {
                ...
                for (IExpr point : inflectionSolutions) {
                    ...
                    analysis.append("  Right concavity: ").append(rightValue > 0 ? "Upwards" : "Downwards").append("\n");
                }
            }
